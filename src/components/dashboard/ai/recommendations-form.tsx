...
const recommendationsSchema = z.object({
  salesData: z.string().min(10, "Sales data is required (e.g., JSON or CSV format)"),
  currentInventory: z.string().min(10, "Current inventory data is required (e.g., JSON or CSV format)"),
...
export function AIRecommendationsForm({ onSubmit, isGenerating, defaultStoreName = "My Store" }: AIRecommendationsFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handleFormSubmit: SubmitHandler<RecommendationsFormInputs> = async (data) => {
    await onSubmit(data);
  };
  ...
  return (
    <Card className="w-full shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Provide your sales data and current inventory levels (in JSON or CSV format) to get AI-driven recommendations.
        </CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.salesData && (
              <p className="text-sm text-destructive">{errors.salesData.message}</p>
            )}
          </div>
          <div>
            <Label htmlFor="currentInventory">Current Inventory (JSON/CSV)</Label>
            <Textarea
              id="currentInventory"
              rows={8}
            ...
            {errors.currentInventory && (
              <p className="text-sm text-destructive">{errors.currentInventory.message}</p>
            )}
          </div>
        </CardContent>
        <CardFooter>
