...
export function CashierOverview() {
  ...
  useEffect(() => {
    if (user) {
      const sales = getSalesByCashierId(user.staffAddr);
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <StatsCard title="My Total Sales Value" value={`$${totalMySalesValue.toFixed(2)}`} icon={DollarSign} description="All sales you've recorded." />
        <StatsCard title="My Total Orders" value={totalMyOrders} icon={ShoppingCart} description="Total orders you've processed." />
        <StatsCard title="My Average Sale Value" value={`$${averageSaleValue.toFixed(2)}`} icon={TrendingUp} description="Average value per order." />
      </div>
      ...
      <Card className="shadow-lg">
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {recentSalesToDisplay.length > 0 ? recentSalesToDisplay.map(sale => (
                <TableRow key={sale.id}>
                  <TableCell className="font-medium">{sale.id.substring(0,8)}...</TableCell>
                  <TableCell>{sale.items.length}</TableCell>
                  <TableCell>${sale.totalAmount.toFixed(2)}</TableCell>
                  <TableCell>{new Date(sale.timestamp).toLocaleDateString()}</TableCell>
                  <TableCell><Badge variant="secondary">{sale.paymentMode}</Badge></TableCell>
                </TableRow>
              )) : (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground">You haven't recorded any sales yet.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
