...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user || (user.role !== 'MANAGER' && user.role !== 'SUPER_ADMIN')) {
      ...
      return jsonError("Forbidden. Only managers can view deshelving records.", 403, { 
    ...
    const reason = url.searchParams.get('reason') as DeshelvingReason || undefined;
    const startDate = url.searchParams.get('startDate') ? new Date(url.searchParams.get('startDate')!) : undefined;
    const endDate = url.searchParams.get('endDate') ? new Date(url.searchParams.get('endDate')!) : undefined;
    const page = parseInt(url.searchParams.get('page') || '1');
    const pageSize = Math.min(parseInt(url.searchParams.get('pageSize') || '20'), 100); // Max 100 per page
