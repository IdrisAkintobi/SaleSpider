...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user || (user.role !== 'MANAGER' && user.role !== 'SUPER_ADMIN')) {
      ...
      return jsonError("Forbidden. Only managers can view deshelving analytics.", 403, { 
    ...
    const daysBack = Math.min(parseInt(url.searchParams.get('daysBack') || '30'), 365); // Max 1 year
