    ...
    const response = {
      ...
      metadata: {
        ...
        performanceMetrics: {
          ...
          productsWithSales: productPerformance.filter(
            p => p.totalQuantitySold > 0
          ).length,
          productsWithoutSales: productPerformance.filter(
            p => p.totalQuantitySold === 0
          ).length,
        },
      },
    ...
    const response = {
      ...
      metadata: {
        ...
        dataRange: {
          ...
          startDate: new Date(
            Date.now() - limitedDaysBack * 24 * 60 * 60 * 1000
          ).toISOString(),
          endDate: new Date().toISOString(),
        },
        customParameters: {
