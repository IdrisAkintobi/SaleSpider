...
import { DUMMY_PRODUCTS, addProduct, updateProductStock, getAllProducts } from "@/lib/data";
import type { Product } from "@/lib/types";
import { useAuth } from "@/contexts/auth-context";
import { PlusCircle, Edit3, PackageSearch, AlertTriangle, CheckCircle2 } from "lucide-react";
...
const productSchema = z.object({
  ...
  lowStockMargin: z.number().int().min(0, "Low stock margin cannot be negative"),
...
export default function InventoryPage() {
  ...
  const handleUpdateStock: SubmitHandler<StockUpdateFormData> = (data) => {
    if (selectedProduct) {
      ...
      setProducts(getAllProducts()); // Refresh products list
      toast({ title: "Stock Updated", description: `Stock for ${selectedProduct.name} updated to ${data.quantity}.` });
                  ...
                  <FormField label="Price ($)" error={errors.price?.message}>
                   ...
                   <FormField label="Initial Quantity" error={errors.quantity?.message}>
                     <Controller
                        name="quantity"
                        control={control}
                        render={({ field }) => <Input id="quantity" type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value,10))} />}
                    />
                  </FormField>
                  <FormField label="Low Stock Margin" error={errors.lowStockMargin?.message}>
                     <Controller
                        name="lowStockMargin"
                        control={control}
                        render={({ field }) => <Input id="lowStockMargin" type="number" {...field}  onChange={e => field.onChange(parseInt(e.target.value,10))}/>}
                    />
                  </FormField>
                  <FormField label="Image URL (Optional)" error={errors.imageUrl?.message}>
                    <Input id="imageUrl" {...register("imageUrl")} placeholder="https://placehold.co/300x300.png" />
                  </FormField>
                  <DialogFooter>
          ...
          <Table>
            ...
            <TableBody>
              {filteredProducts.length > 0 ? filteredProducts.map(product => (
                <TableRow key={product.id}>
                  ...
                  {role === "Manager" && (
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" onClick={() => handleOpenUpdateDialog(product)}>
                ...
                <TableRow>
                  <TableCell colSpan={role === "Manager" ? 7 : 6} className="h-24 text-center">
                    No products found.
                  </TableCell>
                </TableRow>
              )}
      ...
      {selectedProduct && (
        <Dialog open={isUpdateDialogOpen} onOpenChange={setIsUpdateDialogOpen}>
          <DialogContent className="sm:max-w-[425px]">
            ...
            <form onSubmit={stockForm.handleSubmit(handleUpdateStock)} className="grid gap-4 py-4">
              <FormField label="New Quantity" error={stockForm.formState.errors.quantity?.message}>
                <Controller
                    name="quantity"
                    control={stockForm.control}
                    render={({ field }) => <Input id="updateQuantity" type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value,10))} />}
                />
              </FormField>
              <DialogFooter>
                <DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose>
...
function FormField({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-4 items-center gap-4">
      <Label htmlFor={children && (children as React.ReactElement).props.id} className="text-right">
        {label}
      </Label>
