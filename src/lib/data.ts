...
import type { User, Product, Sale, Role, PaymentMode, UserStatus } from "./types";

// Mock Users
export const DUMMY_USERS: User[] = [
  {
    id: "user_manager_001",
    staffAddr: "0xManager001Address noncommutativeEthereu", // Example Ethereum-like address
...
// Mock Sales
export let DUMMY_SALES: Sale[] = [
  {
    id: "sale_001",
    cashierId: DUMMY_USERS[1].staffAddr, // Bob The Builder
    cashierName: DUMMY_USERS[1].name,
    items: [
      { productId: DUMMY_PRODUCTS[0].id, productName: DUMMY_PRODUCTS[0].name, quantity: 2, price: DUMMY_PRODUCTS[0].price },
      { productId: DUMMY_PRODUCTS[1].id, productName: DUMMY_PRODUCTS[1].name, quantity: 1, price: DUMMY_PRODUCTS[1].price },
    ],
    totalAmount: (2 * 25.99) + 75.00,
  ...
  },
  {
    ...
    items: [
      { productId: DUMMY_PRODUCTS[3].id, productName: DUMMY_PRODUCTS[3].name, quantity: 1, price: DUMMY_PRODUCTS[3].price },
    ],
    totalAmount: 45.99,
  ...
  },
  {
    ...
    cashierName: DUMMY_USERS[1].name,
    items: [
      { productId: DUMMY_PRODUCTS[2].id, productName: DUMMY_PRODUCTS[2].name, quantity: 1, price: DUMMY_PRODUCTS[2].price },
    ],
    totalAmount: 250.50,
  ...
  // Add more sales for weekly/daily performance
   {
    ...
    cashierName: DUMMY_USERS[1].name,
    items: [{ productId: DUMMY_PRODUCTS[0].id, productName: DUMMY_PRODUCTS[0].name, quantity: 1, price: DUMMY_PRODUCTS[0].price }],
    totalAmount: 25.99,
  ...
  },
  {
    ...
    cashierName: DUMMY_USERS[2].name,
    items: [{ productId: DUMMY_PRODUCTS[1].id, productName: DUMMY_PRODUCTS[1].name, quantity: 1, price: DUMMY_PRODUCTS[1].price }],
    totalAmount: 75.00,
...
export const updateUserStatus = (staffAddr: string, status: UserStatus): boolean => {
  const userIndex = DUMMY_USERS.findIndex(user => user.staffAddr === staffAddr);
  if (userIndex !== -1) {
...
export const addProduct = (product: Omit<Product, 'id' | 'dateAdded'>): Product => {
  const newProduct: Product = {
    ...product,
    id: `prod_new_${Date.now()}_${Math.random().toString(16).slice(2)}`,
    dateAdded: Date.now(),
    imageUrl: product.imageUrl || `https://placehold.co/300x300.png?text=${encodeURIComponent(product.name)}`
  };
  DUMMY_PRODUCTS.unshift(newProduct); // Add to the beginning
  return newProduct;
};

export const updateProductStock = (id: string, newQuantity: number): boolean => {
