...
export async function exportAuditLogsCSV(filters?: any): Promise<void> {
  try {
    ...
    const columns: CSVColumn[] = [
      ...
      { key: 'userEmail', label: 'User Email', formatter: (value) => value || 'System' },
      { key: 'changes', label: 'Changes', formatter: (value) => value ? JSON.stringify(value) : '' },
      { key: 'oldValues', label: 'Old Values', formatter: (value) => value ? JSON.stringify(value) : '' },
      { key: 'newValues', label: 'New Values', formatter: (value) => value ? JSON.stringify(value) : '' },
      { key: 'metadata', label: 'Metadata', formatter: (value) => value ? JSON.stringify(value) : '' },
    ];
...
export async function exportSalesCSV(filters?: any): Promise<void> {
  try {
    ...
    const columns: CSVColumn[] = [
      { key: 'timestamp', label: 'Date', formatter: (value) => new Date(value).toLocaleDateString() + ' ' + new Date(value).toLocaleTimeString() },
      { key: 'id', label: 'Sale ID' },
      { key: 'cashierName', label: 'Cashier' },
      { key: 'itemsCount', label: 'Items Count', formatter: (_, row) => String(row.items?.length || 0) },
      { key: 'totalAmount', label: 'Total Amount', formatter: formatCurrencyForCSV },
      { key: 'paymentMode', label: 'Payment Method' },
      { key: 'items', label: 'Items Details', formatter: (items) => 
        items?.map((item: any) => `${item.productName} (${item.quantity}x ${item.price})`).join('; ') || ''
      },
    ];
