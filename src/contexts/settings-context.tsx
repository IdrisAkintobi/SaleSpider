...
const defaultSettings: AppSettings = {
  ...
  primaryColor:
    process.env.NEXT_PUBLIC_PRIMARY_COLOR || DEFAULT_SETTINGS.primaryColor,
  secondaryColor:
    process.env.NEXT_PUBLIC_SECONDARY_COLOR || DEFAULT_SETTINGS.secondaryColor,
  accentColor:
    process.env.NEXT_PUBLIC_ACCENT_COLOR || DEFAULT_SETTINGS.accentColor,
  currency: process.env.NEXT_PUBLIC_CURRENCY || DEFAULT_SETTINGS.currency,
  currencySymbol:
    process.env.NEXT_PUBLIC_CURRENCY_SYMBOL || DEFAULT_SETTINGS.currencySymbol,
  vatPercentage: Number.parseFloat(
    process.env.NEXT_PUBLIC_VAT_PERCENTAGE ||
      DEFAULT_SETTINGS.vatPercentage.toString()
  ),
  timezone: process.env.NEXT_PUBLIC_TIMEZONE || DEFAULT_SETTINGS.timezone,
  dateFormat:
    process.env.NEXT_PUBLIC_DATE_FORMAT || DEFAULT_SETTINGS.dateFormat,
  timeFormat:
    process.env.NEXT_PUBLIC_TIME_FORMAT || DEFAULT_SETTINGS.timeFormat,
  ...
  showDeletedProducts:
    process.env.NEXT_PUBLIC_SHOW_DELETED_PRODUCTS === "true" ||
    DEFAULT_SETTINGS.showDeletedProducts,
  enabledPaymentMethods: process.env.NEXT_PUBLIC_ENABLED_PAYMENT_METHODS
    ? process.env.NEXT_PUBLIC_ENABLED_PAYMENT_METHODS.split(",")
        .map(s => s.trim().toUpperCase())
        .filter((v): v is PaymentMode =>
          (PAYMENT_MODE_VALUES as readonly string[]).includes(v)
        )
