...
const HOUR_WEIGHTS = {
  ...
  8: 0.02, 9: 0.05, 10: 0.08, 11: 0.09, 12: 0.12, // Morning
  13: 0.10, 14: 0.08, 15: 0.07, 16: 0.08, 17: 0.10, // Afternoon
  18: 0.09, 19: 0.07, 20: 0.04, 21: 0.01 // Evening
};
...
function generateSaleItems(products: any[]): Array<{productId: string, quantity: number, price: number}> {
...
export async function seedSales(client: PrismaClient, count: number = 12000) {
  ...
  console.log(`👥 Found ${cashiers.length} cashiers and ${products.length} products`);
  ...
  console.log(`📅 Generating sales from ${startDate.toDateString()} to ${endDate.toDateString()}`);
  ...
  for (let i = 0; i < count; i += batchSize) {
    ...
    try {
      ...
      console.log(`✅ Created sales ${i + 1}-${i + currentBatchSize} (${createdCount}/${count}) - Revenue: ₦${totalRevenue.toFixed(2)}`);

      // Progress update every 500 sales
      if (createdCount % 500 === 0) {
        console.log(`🎯 Milestone: ${createdCount} sales created! Total revenue: ₦${totalRevenue.toFixed(2)}`);
      }
      ...
      console.error(`❌ Error creating sales batch ${i + 1}-${i + currentBatchSize}:`, error);
  ...
  paymentStats.forEach(stat => {
    const percentage = ((stat._count.paymentMode / createdCount) * 100).toFixed(1);
    console.log(`  ${stat.paymentMode}: ${stat._count.paymentMode} sales (${percentage}%) - ₦${stat._sum.totalAmount?.toFixed(2) || '0.00'}`);
  });
  ...
  monthlySales.forEach(month => {
    console.log(`  ${month.month.toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}: ${month.sales_count} sales - ₦${month.total_revenue.toFixed(2)}`);
  });
